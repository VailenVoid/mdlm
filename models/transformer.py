"""Plain bidirectional transformer + FFN denoiser for MDLM.

Same block architecture as the AR model
(models/autoregressive.py): pre-LN attention + GELU MLP,
rotary embeddings, no adaLN / timestep conditioning. The
only difference is bidirectional (non-causal) attention.
"""
...
class TransformerBlock(nn.Module):
  def __init__(
    self, dim, n_heads, mlp_ratio=4, dropout=0.1
  ):
    super().__init__()
...
class Transformer(
  nn.Module, huggingface_hub.PyTorchModelHubMixin
):
  ...
  def forward(self, indices, sigma):
    ...
    Args:
      indices: int torch.Tensor with shape
          (batch_size, diffusion_model_input_length), token ids.
      sigma: float torch.Tensor with shape (batch_size).
          Ignored: this backbone has no time conditioning.
